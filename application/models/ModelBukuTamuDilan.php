...
class ModelBukuTamuDilan extends CI_Model {
	...
	private function _get_datatables_query(){
		if($this->input->post('bulan_register_cari')){ $this->db->where('MONTH(tanggal)', $this->input->post('bulan_register_cari')); }
        if($this->input->post('tahun_register_cari')){ $this->db->where('YEAR(tanggal)', $this->input->post('tahun_register_cari')); }
        if($this->input->post('jabatan_cari')){ $this->db->where('tujuan_jabatan_id', $this->input->post('jabatan_cari')); }
	...
	public function get_seleksi_nomor_index($tahun_seleksi){
		try {
			...
			$this->db->where('YEAR(tanggal)',$tahun_seleksi);
	...
	public function get_register_cetak($periode){
		try {
			return $this->db->query("SELECT * FROM bukutamu WHERE LEFT(tanggal,7)='$periode'");
		} catch (Exception $e) {
			return 0;
		}	
	}
